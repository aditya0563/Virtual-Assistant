...
const Home = () => {
  ...
  const handleTextSubmit = async (e) => {
    e.preventDefault();
    if (!textInput.trim() || !userData) return;
  ...
  useEffect(() => {
    ...
    recognition.onerror = (event) => {
      ...
      isRecognizingRef.current = false;
      if (event.error !== "aborted" && event.error !== "network" && event.error !== "not-allowed" && !isSpeakingRef.current) {
        setTimeout(() => {
          try {
            recognition.start();
          } catch (err) {}
  ...
  return (
    <div className="w-full h-[100vh] bg-gradient-to-t from-black via-[#08001a] to-[#010003] flex justify-center items-center flex-col gap-[15px] overflow-hidden relative pt-[30px]">
      ...
      <form onSubmit={handleTextSubmit} className="w-full max-w-[600px] mt-4 flex px-4 mb-[20px] z-40 relative">
         <input 
            type="text" 
            placeholder="Type a command if voice isn't working..." 
            value={textInput}
            onChange={(e) => setTextInput(e.target.value)}
            className="flex-1 h-[50px] rounded-l-full px-5 outline-none border-2 border-white bg-black/50 backdrop-blur text-white placeholder-gray-400 focus:border-blue-400 transition-all"
         />
         <button type="submit" className="h-[50px] px-6 bg-white text-black font-bold rounded-r-full hover:bg-blue-100 transition-all cursor-pointer">Send</button>
      </form>
    </div>
  );
};
